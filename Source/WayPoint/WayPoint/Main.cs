...
namespace WayPoint
{
    public partial class Main : Form
    {
        ...
        public Main()
        {
            InitializeComponent();
            //使用するサイズを入力
            this.ClientSize = new Size(1300,500);
            //自分自身のフォームを最大化
        ...
        private void Output()
        {
            ...
            {
                //All_rectを出力(上書き)
                using (StreamWriter sw = new StreamWriter(@".\All_Rect.txt", false, Encoding.UTF8))
                {
                    //ステージ回繰り返す
            ...
            {
                //All_rect_exを出力(上書き)
                using (StreamWriter sw = new StreamWriter(@".\All_Rect_ex.txt", false, Encoding.UTF8))
                {
                    //ステージ回繰り返す
            ...
            {
                //All_Pointsを出力(上書き)
                using (StreamWriter sw = new StreamWriter(@".\All_Points.txt", false, Encoding.UTF8))
                {
                    //ステージ回繰り返す
            ...
            {
                //All_Tablesを出力(上書き)
                using (StreamWriter sw = new StreamWriter(@".\All_Table.txt", false, Encoding.UTF8))
                {
                    //ステージ回繰り返す
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            {
                Console.WriteLine($"Debugger is attached! MakeTable.");
                AddObject();
                MakeTable();
                SearchPoint();
            ...
            {
                Console.WriteLine($"Debugger is not attached! Skip MakeTable.");
                Input();
                Route();
            }
        ...
        {
            this.Text = $"Stage {now_stage} | Press Up/Down to Change Stage | Click to Change StartPosition";
        ...
        {
            // delta は +1 または -1 のみを想定
            now_stage = (now_stage + delta + All_Rect.Count) % All_Rect.Count;
        }

        private void Main_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyData == Keys.Up) MoveIndex(1);
            else if (e.KeyData == Keys.Down) MoveIndex(-1);
        }
